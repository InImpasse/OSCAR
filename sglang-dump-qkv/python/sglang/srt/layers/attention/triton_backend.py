...
from sglang.srt.layers.attention.q_rotation import maybe_apply_q_rotation, maybe_apply_inv_v_rotation
...
class TritonAttnBackend(AttentionBackend):
    def __init__(
        ...
        # CoQuant: per-layer counter for SGLANG_DUMP_ATTN_INPUT (Q + K-cache snapshot)
        self._dump_attn_chunk_idx = {}
        if (
    ...
    def forward_extend(
        ...
        if save_kv_cache:
            forward_batch.token_to_kv_pool.set_kv_buffer(
                layer, forward_batch.out_cache_loc, k, v, forward_batch=forward_batch
            )
        ...
        if (
            ...
            if remaining > 0:
                ...
                # Compute per-sequence lengths within this chunk
                chunk_seq_lens = []
        ...
        # CoQuant attention-score dump: capture Q for the current chunk + the FULL K cache
        # state (covering req[0]'s entire seq up to now) so offline we can compute
        # softmax(Q @ K^T / sqrt(d)) per server and diff distributions across servers.
        if (
            get_bool_env_var("SGLANG_DUMP_ATTN_INPUT", "false")
            and forward_batch.forward_mode.is_extend()
        ):
            try:
                _dump_attn_max = get_int_env_var("SGLANG_DUMP_ATTN_MAX_CHUNKS", 8)
                _attn_chunk_idx = self._dump_attn_chunk_idx.get(layer_id, 0)
                if _attn_chunk_idx < _dump_attn_max:
                    _attn_save_dir = os.environ.get("SGLANG_DUMP_LAYER_DIR", ".")
                    tp_rank = get_attention_tp_rank()
                    if tp_rank == 0:
                        seq_lens_cpu = forward_batch.seq_lens.cpu().numpy()
                        req_pool_indices_cpu = forward_batch.req_pool_indices.cpu().numpy()
                        req_idx0 = int(req_pool_indices_cpu[0])
                        cur_seq_len = int(seq_lens_cpu[0])
                        # Slice req_to_token to get this request's full token positions.
                        req_to_token = forward_batch.req_to_token_pool.req_to_token
                        token_positions = req_to_token[req_idx0, :cur_seq_len].long()
                        # Read K/V cache state at those positions (post-fake-quant for variants).
                        layer_idx_local = layer_id - forward_batch.token_to_kv_pool.start_layer
                        k_cache_state = forward_batch.token_to_kv_pool.k_buffer[
                            layer_idx_local
                        ][token_positions].detach()
                        # Slice Q to current chunk's first request's tokens.
                        # extend_seq_lens gives current-chunk length per request.
                        ext_seq = forward_batch.extend_seq_lens.cpu().numpy()
                        q_first = (
                            q[: int(ext_seq[0])]
                            .view(-1, layer.tp_q_head_num, layer.qk_head_dim)
                            .detach()
                        ...
                        out_dir = os.path.join(
                            _attn_save_dir, "attn_input", f"layer_{layer_id}"
                        )
                        os.makedirs(out_dir, exist_ok=True)
                        torch.save(
                            {
                                "q": q_first.to(torch.float16).cpu().contiguous(),
                                "k_cache": k_cache_state.to(torch.float16).cpu().contiguous(),
                                "seq_len": cur_seq_len,
                            },
                            os.path.join(out_dir, f"{_attn_chunk_idx}.pt"),
                        )
                self._dump_attn_chunk_idx[layer_id] = _attn_chunk_idx + 1
            except Exception:
                pass  # don't crash production path on dump errors
    ...
    def forward_decode(
        ...
        if hasattr(kv_pool, "dtype") and kv_pool.dtype in ("int4", "int8"):
            ...
            if kv_pool.dtype == "int4" and _hadamard_v_enabled:
                hadamard_order = _hadamard_order
